...
def pool_or_redis(_closable, server):
    ...
    async def redis_factory(maxsize):
        redis = await factory(server.tcp_address, minsize=1, maxsize=maxsize)
        _closable(redis)
        return redis
...
async def blocking_pop(pool, val):
    ...
    async def blpop():
        with await pool as redis:
            # here v0.3 has bound connection, v1.0 does not;
            res = await redis.blpop("list-key", timeout=2, encoding="utf-8")
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            blocking_pop,
            1,
            marks=pytest.mark.xfail(reason="blpop gets connection first and blocks"),
        ),
        (simple_get_set, 10),
        (pipeline, 10),
        (transaction, 10),
...
async def test_operations(pool_or_redis, test_case, pool_size):
    ...
    redis = await pool_or_redis(pool_size)
        ...
        [asyncio.ensure_future(test_case(redis, i)) for i in range(repeat)]
    )
