...
async def test_xadd(redis, server_bin):
    ...
    assert message[1] == OrderedDict([(b"field1", b"value1"), (b"field2", b"value2")])
...
async def test_xadd_maxlen_exact(redis, server_bin):
    ...
    await asyncio.sleep(0.001)
    message_id3 = await redis.xadd(
        "test_stream", {"f3": "v3"}, max_len=2, exact_len=True
    )
...
async def test_xrange(redis, server_bin):
    ...
    # Test start & stop
    messages = await redis.xrange(stream, start=message_id1, stop=message_id2)
    assert len(messages) == 2

    messages = await redis.xrange(
        stream, start="0000000000000-0", stop="9900000000000-0"
    )
    assert len(messages) == 3
...
async def test_xrevrange(redis, server_bin):
    ...
    # Test start & stop
    messages = await redis.xrevrange(stream, start=message_id2, stop=message_id1)
    assert len(messages) == 2

    messages = await redis.xrevrange(
        stream, start="9900000000000-0", stop="0000000000000-0"
    )
    assert len(messages) == 3
...
async def test_xread_selection(redis, server_bin):
    ...
    messages = await redis.xread([stream], timeout=1, latest_ids=[message_id1])
    assert len(messages) == 2

    messages = await redis.xread([stream], timeout=1, latest_ids=[message_id3])
    assert len(messages) == 0

    messages = await redis.xread(
        [stream], timeout=1, latest_ids=["0000000000000-0"], count=2
    )
    assert len(messages) == 2
...
async def test_xread_blocking(redis, create_redis, server, server_bin):
    ...
    producer_task = asyncio.Task(add_message_with_sleep(redis, "test_stream", fields))
...
async def test_xread_group(redis):
    ...
    messages = await redis.xread_group(
        "test_group", "test_consumer", ["test_stream"], timeout=1000, latest_ids=[">"]
    )
    assert len(messages) == 1
...
async def test_xack_and_xpending(redis):
    ...
    pending_count, min_id, max_id, count = await redis.xpending(
    ...
    # It is now pending
    pending_count, min_id, max_id, pel = await redis.xpending(
        "test_stream", "test_group"
    )
...
async def test_xpending_get_messages(redis):
    ...
    (
        message_id,
        consumer_name,
        milliseconds_since_last_delivery,
        num_deliveries,
    ) = response[0]
...
async def test_xclaim_simple(redis):
    ...
    result = await redis.xclaim(
        "test_stream", "test_group", "new_consumer", min_idle_time=0, id=message_id
    )
    assert result
...
async def test_xclaim_min_idle_time_includes_messages(redis):
    ...
    result = await redis.xclaim(
        "test_stream", "test_group", "new_consumer", min_idle_time=50, id=message_id
    )
    assert result
...
async def test_xgroup_delconsumer(redis, create_redis, server):
    ...
    await redis.xread_group(
        "test_group", "test_consumer", streams=["test_stream"], latest_ids=[0]
    )
