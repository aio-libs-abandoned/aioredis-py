...
async def test_bzpopmax(redis):
    ...
    pairs = [(0, b"a"), (5, b"c"), (2, b"d"), (8, b"e"), (9, b"f"), (3, b"g")]
...
async def test_zcount(redis):
    ...
    res = await redis.zcount(key, 1, 3, exclude=redis.ZSET_EXCLUDE_BOTH)
    assert res == 1
    res = await redis.zcount(key, 1, 3, exclude=redis.ZSET_EXCLUDE_MIN)
    assert res == 2
    res = await redis.zcount(key, 1, 3, exclude=redis.ZSET_EXCLUDE_MAX)
    assert res == 3
    res = await redis.zcount(key, 1, exclude=redis.ZSET_EXCLUDE_MAX)
    assert res == 5
    res = await redis.zcount(key, float("-inf"), 3, exclude=redis.ZSET_EXCLUDE_MIN)
    assert res == 4
...
async def test_zinterstore(redis):
    ...
    res = await redis.zinterstore(
        "zout", "zset1", "zset2", aggregate=redis.ZSET_AGGREGATE_SUM
    )
    assert res == 1
    res = await redis.zrange("zout", withscores=True)
    assert res == [(b"one", 5)]
    ...
    res = await redis.zinterstore(
        "zout", "zset1", "zset2", aggregate=redis.ZSET_AGGREGATE_MIN
    )
    assert res == 1
    res = await redis.zrange("zout", withscores=True)
    assert res == [(b"one", 2)]
    ...
    res = await redis.zinterstore(
        "zout", "zset1", "zset2", aggregate=redis.ZSET_AGGREGATE_MAX
    )
    assert res == 1
    res = await redis.zrange("zout", withscores=True)
    assert res == [(b"one", 3)]
    ...
    with pytest.raises(AssertionError):
        await redis.zinterstore("zout", "zset1", "zset2", with_weights=True)

    res = await redis.zinterstore("zout", ("zset1", 2), ("zset2", 2), with_weights=True)
    assert res == 1
    res = await redis.zrange("zout", withscores=True)
    assert res == [(b"one", 10)]


@redis_version(2, 8, 9, reason="ZLEXCOUNT is available since redis>=2.8.9")
@pytest.mark.asyncio
...
@redis_version(2, 8, 9, reason="ZRANGEBYLEX is available since redis>=2.8.9")
@pytest.mark.asyncio
async def test_zrangebylex(redis):
    ...
    with pytest.raises(TypeError):
        await redis.zrangebylex(key, b"a", b"e", offset="one", count=1)
    with pytest.raises(TypeError):
        await redis.zrangebylex(key, b"a", b"e", offset=1, count="one")
...
async def test_zrangebyscore(redis, encoding):
    ...
    res = await redis.zrangebyscore(key, 1, 7, withscores=False, encoding=encoding)
    assert res == members
    res = await redis.zrangebyscore(
        key, 1, 7, withscores=False, exclude=redis.ZSET_EXCLUDE_BOTH, encoding=encoding
    )
    assert res == members[2:-1]
    res = await redis.zrangebyscore(key, 1, 7, withscores=True, encoding=encoding)
    assert res == rev_pairs

    res = await redis.zrangebyscore(key, 1, 10, offset=2, count=2, encoding=encoding)
...
@redis_version(2, 8, 9, reason="ZREMRANGEBYLEX is available since redis>=2.8.9")
@pytest.mark.asyncio
...
async def test_zremrangebyscore(redis):
    ...
    res = await redis.zremrangebyscore(key, 3, 7.5, exclude=redis.ZSET_EXCLUDE_MIN)
    assert res == 1
    res = await redis.zrange(key, 0, -1)
    assert res == members[:-1]

    res = await redis.zremrangebyscore(key, 1, 3, exclude=redis.ZSET_EXCLUDE_BOTH)
    assert res == 1
    res = await redis.zrange(key, 0, -1)
...
async def test_zrevrange(redis, encoding):
    ...
    res = await redis.zrevrange(key, 0, -1, withscores=False, encoding=encoding)
    assert res == members[::-1]
    res = await redis.zrevrange(key, 0, -1, withscores=True, encoding=encoding)
    assert res == rev_pairs[::-1]
    res = await redis.zrevrange(key, -2, -1, withscores=False, encoding=encoding)
    assert res == members[1::-1]
    res = await redis.zrevrange(key, 1, 2, withscores=False, encoding=encoding)
...
async def test_zunionstore(redis):
    ...
    with pytest.raises(AssertionError):
        await redis.zunionstore("zout", "zset1", "zset2", with_weights=True)

    res = await redis.zunionstore("zout", ("zset1", 2), ("zset2", 2), with_weights=True)
    assert res == 3
    res = await redis.zrange("zout", withscores=True)
...
async def test_zrevrangebyscore(redis, encoding):
    ...
    res = await redis.zrevrangebyscore(key, 7, 1, withscores=False, encoding=encoding)
    assert res == members[::-1]
    res = await redis.zrevrangebyscore(
        key, 7, 1, withscores=False, exclude=redis.ZSET_EXCLUDE_BOTH, encoding=encoding
    )
    assert res == members[-2:1:-1]
    res = await redis.zrevrangebyscore(key, 7, 1, withscores=True, encoding=encoding)
    assert res == rev_pairs

    res = await redis.zrevrangebyscore(key, 10, 1, offset=2, count=2, encoding=encoding)
...
@redis_version(2, 8, 9, reason="ZREVRANGEBYLEX is available since redis>=2.8.9")
@pytest.mark.asyncio
async def test_zrevrangebylex(redis):
    ...
    with pytest.raises(TypeError):
        await redis.zrevrangebylex(key, b"a", b"e", offset="one", count=1)
    with pytest.raises(TypeError):
        await redis.zrevrangebylex(key, b"a", b"e", offset=1, count="one")
