...
async def add(redis, key, field, value):
    ok = await redis.connection.execute(b"hset", key, field, value)
    assert ok == 1
...
async def test_hgetall(redis):
    ...
    test_value = await redis.hgetall("key:hgetall", encoding="utf-8")
...
async def test_hmget(redis):
    ...
    test_value = await redis.hmget(key, b"not:" + field1, b"not:" + field2)
    assert [None, None] == test_value

    val = await redis.hincrby(key, "numeric")
    assert val == 1
    test_value = await redis.hmget(key, field1, field2, "numeric", encoding="utf-8")
