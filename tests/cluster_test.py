...
async def test_cluster_setslots_ok(test_cluster):
    ...
    """
    masters = test_cluster.master_nodes
    master_from = masters[0]
    master_to = masters[1]

    slot_to_move = master_from.slots[0][0]

    # Set node to migrate
    res = await test_cluster.cluster_setslot(
        slot_to_move,
        'migrating',
        master_to.id,
        address=master_from.address
    )
    assert res

    for node in await test_cluster.cluster_nodes(address=master_from.address):
        if node['id'] == master_from.id:
            break

    assert node['migrations']
    assert node['migrations'][0] == {
        'slot': slot_to_move,
        'node_id': master_to.id,
        'state': 'migrating'
    }
    ...
    res = await test_cluster.cluster_setslot(
        ...
        'importing',
        master_from.id,
        address=master_to.address
    )
    assert res

    for node in await test_cluster.cluster_nodes(address=master_to.address):
        if node['id'] == master_to.id:
            break

    assert node['migrations']
    assert node['migrations'][0] == {
        'slot': slot_to_move,
        'node_id': master_from.id,
        'state': 'importing'
    }
    ...
    from_slots = list(master_from.slots[0])
    from_slots[0] = from_slots[0] + 1
    from_slots = tuple(from_slots)

    to_slots = list(master_to.slots[0])
    if slot_to_move == to_slots[0] - 1:
        to_slots[0] = slot_to_move
        to_slots = tuple(to_slots)
    elif slot_to_move == to_slots[-1] + 1:
        to_slots[-1] = slot_to_move
        to_slots = tuple(to_slots)
    else:
        to_slots = (slot_to_move, slot_to_move, )

    async def check_info(address):
        from_node = None
        for node in await test_cluster.cluster_nodes(address=address):
            if node['id'] == master_from.id:
                from_node = node
                assert node['slots'] == (tuple(from_slots), )

            if node['id'] == master_to.id:
                assert not node['migrations']
                assert to_slots in node['slots']
        return from_node

    await check_info(master_to.address)
    # We need to wait while changes will affect both nodes
    await asyncio.sleep(2)
    node = await check_info(master_from.address)

    assert node['migrations']

    # Fixing state
    # @see https://redis.io/commands/cluster-setslot#
    # cluster-setslot-codeltslotgtcode-stable
    res = await test_cluster.cluster_setslot(
        slot_to_move,
        'stable',
        address=master_from.address
...
async def test_cluster_setslots_fail(test_cluster, key_and_slot):
    ...
    for node in test_cluster.master_nodes:
        if node.in_range(slot_with_key):
            slot_node = node
        elif not other_node:
            other_node = node

    slot = slot_node.slots[-1][-1]

    with pytest.raises(ReplyError):
        await test_cluster.cluster_setslot(
            slot_with_key,
            'node',
            other_node.id,
            address=slot_node.address
        )
