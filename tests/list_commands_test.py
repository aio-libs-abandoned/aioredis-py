...
async def test_blpop_blocking_features(redis, create_redis, server):
    ...
    producer_task = asyncio.ensure_future(push_data_with_sleep(redis, key2, value))
...
async def test_brpoplpush(redis):
    ...
    # test encoding param
    result = await redis.brpoplpush(destkey, key, encoding="utf-8")
...
async def test_brpoplpush_blocking_features(redis, create_redis, server):
    ...
    producer_task = asyncio.ensure_future(push_data_with_sleep(redis, source, value))
    results = await asyncio.gather(consumer_task, producer_task)
...
async def test_linsert(redis):
    ...
    test_value = await redis.linsert(key, value2, value3, before=True)
    assert test_value == 3
    # insert element after pivot
    test_value = await redis.linsert(key, value2, value4, before=False)
    assert test_value == 4
    ...
    test_value = await redis.linsert(key, b"not:pivot", value3, before=True)
    assert test_value == -1
...
async def test_rpoplpush(redis):
    ...
    # test encoding param
    result = await redis.rpoplpush(destkey, key, encoding="utf-8")
