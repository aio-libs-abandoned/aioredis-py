...
@pytest.mark.parametrize(
    "url,expected_address,expected_options",
    [
        # redis scheme
        ...
        ("redis://user:secret@localhost", ("localhost", 6379), {"password": "secret"}),
        (
        ...
        ("redis://localhost?timeout=1.0", ("localhost", 6379), {"timeout": 1.0}),
        ("redis://localhost?timeout=10", ("localhost", 6379), {"timeout": 10.0}),
        # rediss scheme
        ...
        ("rediss://localhost:6379?db=1", ("localhost", 6379), {"ssl": True, "db": 1}),
        ("rediss://localhost:6379/?db=1", ("localhost", 6379), {"ssl": True, "db": 1}),
        ...
        (
            ...
            ("localhost", 6379),
            {"ssl": True, "encoding": "utf-8"},
        ),
        (
            "rediss://localhost?timeout=1.0",
            ("localhost", 6379),
            {"ssl": True, "timeout": 1.0},
        ),
        (
            "rediss://localhost?timeout=10",
            ("localhost", 6379),
            {"ssl": True, "timeout": 10.0},
        ),
        # unix scheme
        ("unix:///", "/", {}),
        ("unix:///redis.sock?db=12", "/redis.sock", {"db": 12}),
        ("unix:///redis.sock?encoding=utf-8", "/redis.sock", {"encoding": "utf-8"}),
        ("unix:///redis.sock?ssl=true", "/redis.sock", {"ssl": True}),
        ("unix:///redis.sock?timeout=12", "/redis.sock", {"timeout": 12}),
        # no scheme
...
@pytest.mark.parametrize(
    "url,expected_error",
    [
        ("bad-scheme://localhost:6379/", ("Unsupported URI scheme", "bad-scheme")),
        ("redis:///?db=1&db=2", ("Multiple parameters are not allowed", "db", "2")),
        ("redis:///?db=", ("Empty parameters are not allowed", "db", "")),
        ("redis:///?foo=", ("Empty parameters are not allowed", "foo", "")),
        ("unix://", ("Empty path is not allowed", "unix://")),
        (
            "unix://host:123/",
        ...
        ("redis:///01", ("Expected integer without leading zeroes", "01")),
        ("rediss:///01", ("Expected integer without leading zeroes", "01")),
        ("redis:///?db=01", ("Expected integer without leading zeroes", "01")),
        ("rediss:///?db=01", ("Expected integer without leading zeroes", "01")),
        ("redis:///1?db=2", ("Single DB value expected, got path and query", 1, 2)),
        ("rediss:///1?db=2", ("Single DB value expected, got path and query", 1, 2)),
        (
            "redis://:passwd@localhost/?password=passwd",
            ("Single password value is expected, got in net location and query"),
        ),
        ("redis:///?ssl=1", ("Expected 'ssl' param to be 'true' or 'false' only", "1")),
        (
            "redis:///?ssl=True",
