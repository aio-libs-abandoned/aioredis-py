...
@redis_version(3, 2, 0, reason="GEODIST is available since redis >= 3.2.0")
@pytest.mark.asyncio
...
@redis_version(3, 2, 0, reason="GEOHASH is available since redis >= 3.2.0")
@pytest.mark.asyncio
async def test_geohash(redis):
    ...
    res = await redis.geohash("geodata", "Palermo", encoding="utf-8")
    assert res == ["sqc8b49rny0"]

    res = await redis.geohash("geodata", "Palermo", "Catania", encoding="utf-8")
    assert res == ["sqc8b49rny0", "sqdtr74hyu0"]


@redis_version(3, 2, 0, reason="GEOPOS is available since redis >= 3.2.0")
@pytest.mark.asyncio
...
@redis_version(3, 2, 0, reason="GEO* is available since redis >= 3.2.0")
@pytest.mark.asyncio
...
@redis_version(3, 2, 0, reason="GEORADIUS is available since redis >= 3.2.0")
@pytest.mark.asyncio
async def test_georadius_validation(redis):
    ...
    with pytest.raises(TypeError):
        res = await redis.georadius("geodata", 15, 37, "200", "km", encoding="utf-8")
    with pytest.raises(ValueError):
        res = await redis.georadius("geodata", 15, 37, 200, "k", encoding="utf-8")
    with pytest.raises(ValueError):
        res = await redis.georadius(
...
async def test_georadius(redis):
    ...
    res = await redis.georadius("geodata", 15, 37, 200, "km", count=1, encoding="utf-8")
    assert res == ["Catania"]
    ...
    res = await redis.georadius(
        "geodata", 15, 37, 200, "km", with_dist=True, with_coord=True, encoding="utf-8"
    )
    assert res == [
        GeoMember(
            member="Palermo",
    ...
    res = await redis.georadius(
        "geodata", 15, 37, 200, "km", with_coord=True, with_hash=True, encoding="utf-8"
    )
    assert res == [
        GeoMember(
            member="Palermo",
    ...
    assert res == [
        GeoMember(member="Palermo", dist=None, hash=3479099956230698, coord=None)
    ]


@redis_version(3, 2, 0, reason="GEORADIUSBYMEMBER is available since redis >= 3.2.0")
@pytest.mark.asyncio
...
async def test_geohash_binary(redis):
    ...
    res = await redis.geohash("geodata", "Palermo")
    assert res == [b"sqc8b49rny0"]

    res = await redis.geohash("geodata", "Palermo", "Catania")
...
async def test_georadiusbymember_binary(redis):
    ...
    res = await redis.georadiusbymember("geodata", "Palermo", 200, "km", with_dist=True)
    assert res == [
    ...
    res = await redis.georadiusbymember(
        "geodata", "Palermo", 200, "km", with_dist=True, with_coord=True
    )
    assert res == [
        GeoMember(
            member=b"Palermo",
    ...
    res = await redis.georadiusbymember(
        "geodata", "Palermo", 200, "km", with_dist=True, with_coord=True, with_hash=True
    )
    assert res == [
        GeoMember(
            member=b"Palermo",
