...
@pytest.fixture()
async def slowlog(r: aioredis.Redis, event_loop):
...
async def redis_server_time(client: aioredis.Redis):
    seconds, milliseconds = await client.time()
    timestamp = float(f"{seconds}.{milliseconds}")
    return datetime.datetime.fromtimestamp(timestamp)


async def get_stream_message(client: aioredis.Redis, stream: str, message_id: str):
...
class TestResponseCallbacks:
    ...
    async def test_response_callbacks(self, r: aioredis.Redis):
        ...
        assert id(r.response_callbacks) != id(aioredis.Redis.RESPONSE_CALLBACKS)
        r.set_response_callback("GET", lambda x: "static")
        await r.set("a", "foo")
        assert await r.get("a") == "static"

    async def test_case_insensitive_command_names(self, r: aioredis.Redis):
        assert r.response_callbacks["del"] == r.response_callbacks["DEL"]


class TestRedisCommands:
    async def test_command_on_invalid_key_type(self, r: aioredis.Redis):
        await r.lpush("a", "1")
    ...
    @skip_if_redis_enterprise
    async def test_client_list_replica(self, r):
    ...
    @skip_if_server_version_lt("6.2.0")
    async def test_client_kill_filter_by_laddr(
        self, r: aioredis.Redis, r2: aioredis.Redis
    ):
        await r.client_setname("redis-py-c1")
        await r2.client_setname("redis-py-c2")
        clients = [
    ...
    @skip_if_server_version_lt("6.0.0")
    @skip_if_redis_enterprise
    async def test_client_kill_filter_by_user(self, r: aioredis.Redis, create_redis):
    ...
    async def test_client_getredir(self, r):
        assert isinstance(await r.client_getredir(), int)
        assert await r.client_getredir() == -1
    ...
    async def test_decrby(self, r: aioredis.Redis):
        assert await r.decrby("a", amount=2) == -2
        assert await r.decrby("a", amount=3) == -5
        assert await r.get("a") == b"-5"

    async def test_delete(self, r: aioredis.Redis):
        assert await r.delete("a") == 0
        await r.set("a", "foo")
        assert await r.delete("a") == 1

    async def test_delete_with_multiple_keys(self, r: aioredis.Redis):
        await r.set("a", "foo")
        await r.set("b", "bar")
    ...
    async def test_delitem(self, r: aioredis.Redis):
        await r.set("a", "foo")
        await r.delete("a")
        assert await r.get("a") is None
    ...
    async def test_exists(self, r: aioredis.Redis):
    ...
    async def test_exists_contains(self, r: aioredis.Redis):
        assert not await r.exists("a")
        await r.set("a", "foo")
        assert await r.exists("a")

    async def test_expire(self, r: aioredis.Redis):
    ...
    async def test_expireat_datetime(self, r: aioredis.Redis):
        expire_at = await redis_server_time(r) + datetime.timedelta(minutes=1)
        await r.set("a", "foo")
        assert await r.expireat("a", expire_at)
        assert 0 < await r.ttl("a") <= 61

    async def test_expireat_no_key(self, r: aioredis.Redis):
        expire_at = await redis_server_time(r) + datetime.timedelta(minutes=1)
        assert not await r.expireat("a", expire_at)

    async def test_expireat_unixtime(self, r: aioredis.Redis):
        expire_at = await redis_server_time(r) + datetime.timedelta(minutes=1)
        await r.set("a", "foo")
    ...
    async def test_get_and_set(self, r: aioredis.Redis):
    ...
    async def test_incrby(self, r: aioredis.Redis):
    ...
    async def test_set_px(self, r: aioredis.Redis):
        ...
        assert 0 < await r.ttl("a") <= 10
        with pytest.raises(exceptions.DataError):
            assert await r.set("a", "1", px=10.0)
    ...
    async def test_set_ex(self, r: aioredis.Redis):
        ...
        assert 0 < await r.ttl("a") <= 10
        with pytest.raises(exceptions.DataError):
            assert await r.set("a", "1", ex=10.0)
    ...
    async def test_setnx(self, r: aioredis.Redis):
        assert await r.setnx("a", "1")
        assert await r.get("a") == b"1"
        assert not await r.setnx("a", "2")
        assert await r.get("a") == b"1"

    async def test_setrange(self, r: aioredis.Redis):
    ...
    async def test_substr(self, r: aioredis.Redis):
    ...
    async def test_ttl(self, r: aioredis.Redis):
        await r.set("a", "1")
        assert await r.expire("a", 10)
        assert 0 < await r.ttl("a") <= 10
        assert await r.persist("a")
    ...
    async def test_type(self, r: aioredis.Redis):
    ...
    async def test_llen(self, r: aioredis.Redis):
        await r.rpush("a", "1", "2", "3")
        assert await r.llen("a") == 3

    async def test_lpop(self, r: aioredis.Redis):
        await r.rpush("a", "1", "2", "3")
    ...
    async def test_lpushx(self, r: aioredis.Redis):
    ...
    async def test_lrem(self, r: aioredis.Redis):
    ...
    async def test_rpush(self, r: aioredis.Redis):
    ...
    async def test_rpushx(self, r: aioredis.Redis):
    ...
    # SET COMMANDS
    async def test_sadd(self, r: aioredis.Redis):
        members = {b"1", b"2", b"3"}
        await r.sadd("a", *members)
        assert await r.smembers("a") == members

    async def test_scard(self, r: aioredis.Redis):
        await r.sadd("a", "1", "2", "3")
    ...
    async def test_sismember(self, r: aioredis.Redis):
        await r.sadd("a", "1", "2", "3")
    ...
    async def test_smembers(self, r: aioredis.Redis):
        await r.sadd("a", "1", "2", "3")
    ...
    async def test_spop(self, r: aioredis.Redis):
        s = [b"1", b"2", b"3"]
        await r.sadd("a", *s)
    ...
    async def test_srandmember(self, r: aioredis.Redis):
        s = [b"1", b"2", b"3"]
        await r.sadd("a", *s)
    ...
    async def test_srem(self, r: aioredis.Redis):
    ...
    async def test_zadd_nx(self, r: aioredis.Redis):
    ...
    async def test_zadd_incr(self, r: aioredis.Redis):
        assert await r.zadd("a", {"a1": 1}) == 1
        assert await r.zadd("a", {"a1": 4.5}, incr=True) == 5.5

    async def test_zadd_incr_with_xx(self, r: aioredis.Redis):
    ...
    async def test_zcount(self, r: aioredis.Redis):
        await r.zadd("a", {"a1": 1, "a2": 2, "a3": 3})
    ...
    async def test_zrange(self, r: aioredis.Redis):
        ...
        assert await r.zrange("a", 0, 2) == [b"a1", b"a2", b"a3"]
        assert await r.zrange("a", 0, 2, desc=True) == [b"a3", b"a2", b"a1"]
    ...
    async def test_zrange_errors(self, r):
        with pytest.raises(exceptions.DataError):
            await r.zrange("a", 0, 1, byscore=True, bylex=True)
        with pytest.raises(exceptions.DataError):
            await r.zrange("a", 0, 1, bylex=True, withscores=True)
        with pytest.raises(exceptions.DataError):
            await r.zrange("a", 0, 1, byscore=True, withscores=True, offset=4)
        with pytest.raises(exceptions.DataError):
            await r.zrange("a", 0, 1, byscore=True, withscores=True, num=2)

    @skip_if_server_version_lt("6.2.0")
    async def test_zrange_params(self, r):
        # bylex
        await r.zadd("a", {"a": 0, "b": 0, "c": 0, "d": 0, "e": 0, "f": 0, "g": 0})
        assert await r.zrange("a", "[aaa", "(g", bylex=True) == [
            b"b",
            b"c",
            b"d",
            b"e",
        ...
        assert await r.zrange("a", "[f", "+", bylex=True) == [b"f", b"g"]
        assert await r.zrange("a", "+", "[f", desc=True, bylex=True) == [b"g", b"f"]
        assert await r.zrange("a", "-", "+", bylex=True, offset=3, num=2) == [
            b"d",
            b"e",
        ]
        assert await r.zrange(
            "a", "+", "-", desc=True, bylex=True, offset=3, num=2
        ) == [b"d", b"c"]

        # byscore
        await r.zadd("a", {"a1": 1, "a2": 2, "a3": 3, "a4": 4, "a5": 5})
        assert await r.zrange("a", 2, 4, byscore=True, offset=1, num=2) == [
            b"a3",
            b"a4",
        ]
        assert await r.zrange("a", 4, 2, desc=True, byscore=True, offset=1, num=2) == [
            b"a3",
            b"a2",
        ]
        assert await r.zrange("a", 2, 4, byscore=True, withscores=True) == [
            (b"a2", 2.0),
            (b"a3", 3.0),
            (b"a4", 4.0),
        ]
    ...
    async def test_zrangebyscore(self, r: aioredis.Redis):
        await r.zadd("a", {"a1": 1, "a2": 2, "a3": 3, "a4": 4, "a5": 5})
    ...
    async def test_zrem_multiple_keys(self, r: aioredis.Redis):
        await r.zadd("a", {"a1": 1, "a2": 2, "a3": 3})
    ...
    async def test_zremrangebyrank(self, r: aioredis.Redis):
        await r.zadd("a", {"a1": 1, "a2": 2, "a3": 3, "a4": 4, "a5": 5})
        assert await r.zremrangebyrank("a", 1, 3) == 3
        assert await r.zrange("a", 0, 5) == [b"a1", b"a5"]

    async def test_zremrangebyscore(self, r: aioredis.Redis):
        await r.zadd("a", {"a1": 1, "a2": 2, "a3": 3, "a4": 4, "a5": 5})
    ...
    async def test_zrevrange(self, r: aioredis.Redis):
        await r.zadd("a", {"a1": 1, "a2": 2, "a3": 3})
    ...
    async def test_zrevrangebyscore(self, r: aioredis.Redis):
        await r.zadd("a", {"a1": 1, "a2": 2, "a3": 3, "a4": 4, "a5": 5})
    ...
    async def test_zunion(self, r: aioredis.Redis):
        ...
        await r.zadd("c", {"a1": 6, "a3": 5, "a4": 4})
        # sum
        assert await r.zunion(["a", "b", "c"]) == [b"a2", b"a4", b"a3", b"a1"]
    ...
    # HASH COMMANDS
    async def test_hget_and_hset(self, r: aioredis.Redis):
    ...
    async def test_hset_with_multi_key_values(self, r: aioredis.Redis):
        await r.hset("a", mapping={"1": 1, "2": 2, "3": 3})
        assert await r.hget("a", "1") == b"1"
        assert await r.hget("a", "2") == b"2"
        assert await r.hget("a", "3") == b"3"
    ...
    async def test_hdel(self, r: aioredis.Redis):
        await r.hset("a", mapping={"1": 1, "2": 2, "3": 3})
    ...
    async def test_hexists(self, r: aioredis.Redis):
        await r.hset("a", mapping={"1": 1, "2": 2, "3": 3})
        assert await r.hexists("a", "1")
        assert not await r.hexists("a", "4")

    async def test_hgetall(self, r: aioredis.Redis):
        h = {b"a1": b"1", b"a2": b"2", b"a3": b"3"}
        await r.hset("a", mapping=h)
        assert await r.hgetall("a") == h

    async def test_hincrby(self, r: aioredis.Redis):
    ...
    async def test_hkeys(self, r: aioredis.Redis):
        h = {b"a1": b"1", b"a2": b"2", b"a3": b"3"}
        await r.hset("a", mapping=h)
    ...
    async def test_hlen(self, r: aioredis.Redis):
        await r.hset("a", mapping={"1": 1, "2": 2, "3": 3})
        assert await r.hlen("a") == 3

    async def test_hmget(self, r: aioredis.Redis):
    ...
    async def test_hsetnx(self, r: aioredis.Redis):
    ...
    async def test_hvals(self, r: aioredis.Redis):
        h = {b"a1": b"1", b"a2": b"2", b"a3": b"3"}
        await r.hset("a", mapping=h)
    ...
    # SORT
    async def test_sort_basic(self, r: aioredis.Redis):
        await r.rpush("a", "3", "2", "1", "4")
        assert await r.sort("a") == [b"1", b"2", b"3", b"4"]

    async def test_sort_limited(self, r: aioredis.Redis):
        await r.rpush("a", "3", "2", "1", "4")
    ...
    async def test_sort_groups_no_get(self, r: aioredis.Redis):
        await r.set("user:1", "u1")
        await r.set("user:2", "u2")
        await r.set("user:3", "u3")
        await r.rpush("a", "2", "3", "1")
    ...
    async def test_sort_desc(self, r: aioredis.Redis):
        await r.rpush("a", "2", "3", "1")
        assert await r.sort("a", desc=True) == [b"3", b"2", b"1"]

    async def test_sort_alpha(self, r: aioredis.Redis):
    ...
    async def test_sort_issue_924(self, r: aioredis.Redis):
    ...
    @skip_if_server_version_lt("6.2.0")
    async def test_geoadd_nx(self, r):
        values = (2.1909389952632, 41.433791470673, "place1") + (
            2.1873744593677,
            41.406342043777,
            "place2",
        )
        assert await r.geoadd("a", values) == 2
        values = (
            (2.1909389952632, 41.433791470673, "place1")
            + (2.1873744593677, 41.406342043777, "place2")
            + (2.1804738294738, 41.405647879212, "place3")
        )
        assert await r.geoadd("a", values, nx=True) == 1
        assert await r.zrange("a", 0, -1) == [b"place3", b"place2", b"place1"]

    @skip_if_server_version_lt("6.2.0")
    async def test_geoadd_xx(self, r):
        values = (2.1909389952632, 41.433791470673, "place1")
        assert await r.geoadd("a", values) == 1
        values = (2.1909389952632, 41.433791470673, "place1") + (
            2.1873744593677,
            41.406342043777,
            "place2",
        )
        assert await r.geoadd("a", values, xx=True) == 0
        assert await r.zrange("a", 0, -1) == [b"place1"]

    @skip_if_server_version_lt("6.2.0")
    async def test_geoadd_ch(self, r):
        values = (2.1909389952632, 41.433791470673, "place1")
        assert await r.geoadd("a", values) == 1
        ...
        assert await r.geoadd("a", values, ch=True) == 2
        assert await r.zrange("a", 0, -1) == [b"place1", b"place2"]
    ...
    @skip_if_server_version_lt("6.2.0")
    async def test_georadiusmember_count(self, r):
        values = (2.1909389952632, 41.433791470673, "place1") + (
            2.1873744593677,
            41.406342043777,
            b"\x80place2",
    ...
    async def test_xautoclaim(self, r: aioredis.Redis):
        ...
        response = await r.xautoclaim(
            stream, group, consumer2, min_idle_time=0, count=1
        )
        assert response == [message]

        # reclaim the messages as consumer1, but use the justid argument
        # which only returns message ids
        assert await r.xautoclaim(
    ...
    async def test_xautoclaim_negative(self, r: aioredis.Redis):
        ...
        with pytest.raises(aioredis.DataError):
            await r.xautoclaim(stream, group, consumer, min_idle_time=0, count=-1)
    ...
    @skip_if_server_version_lt("6.0.0")
    async def test_xinfo_stream_full(self, r):
        stream = "stream"
        group = "group"
        m1 = await r.xadd(stream, {"foo": "bar"})
        await r.xgroup_create(stream, group, 0)
    ...
    async def test_xpending_range(self, r: aioredis.Redis):
        ...
        response = await r.xpending_range(
    ...
    async def test_xpending_range_negative(self, r: aioredis.Redis):
        ...
        with pytest.raises(ValueError):
            await r.xpending_range(stream, group, min="-", max="+", count=5, idle="one")
        with pytest.raises(aioredis.exceptions.ResponseError):
            await r.xpending_range(stream, group, min="-", max="+", count=5, idle=1.5)
        with pytest.raises(aioredis.DataError):
            await r.xpending_range(stream, group, min="-", max="+", count=5, idle=-1)
        with pytest.raises(aioredis.DataError):
            await r.xpending_range(
                stream, group, min=None, max=None, count=None, idle=0
            )
        with pytest.raises(aioredis.DataError):
            await r.xpending_range(
    ...
    @skip_if_server_version_lt("5.0.0")
    async def test_restore_idletime(self, r):
        key = "yayakey"
    ...
    @skip_if_server_version_lt("5.0.0")
    async def test_restore_frequency(self, r):
        key = "yayakey"
        await r.set(key, "blee!")
        dumpdata = await r.dump(key)
        await r.delete(key)
...
class TestBinarySave:
    ...
    async def test_binary_lists(self, r: aioredis.Redis):
        mapping = {
    ...
    async def test_22_info(self, r: aioredis.Redis):
        """
    ...
    async def test_large_responses(self, r: aioredis.Redis):
    ...
    async def test_floating_point_encoding(self, r: aioredis.Redis):
        """
