...
from aioredis.util import _NOTSET, wait_convert, wait_make_dict, wait_ok
...
class ServerCommandsMixin:
    ...
    def command_info(self, command, *commands):
        ...
        return self.execute(b"COMMAND", b"INFO", command, *commands, encoding="utf-8")
...
SlaveInfo = namedtuple("SlaveInfo", "role master_ip master_port state received")
...
def parse_role(role):
    type_ = role[0]
    if type_ == "master":
        slaves = [MasterSlaveInfo(s[0], int(s[1]), int(s[2])) for s in role[2]]
