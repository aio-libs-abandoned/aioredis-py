...
class SortedSetCommandsMixin:
    ...
    def zadd(self, key, score, member, *pairs, exist=None, changed=False, incr=False):
    ...
    def zcount(self, key, min=float("-inf"), max=float("inf"), *, exclude=None):
        ...
        return self.execute(b"ZCOUNT", key, *_encode_min_max(exclude, min, max))
    ...
    def zinterstore(self, destkey, key, *keys, with_weights=False, aggregate=None):
        ...
        if with_weights:
            assert all(
                isinstance(val, (list, tuple)) for val in keys
    ...
    def zrange(self, key, start=0, stop=-1, withscores=False, encoding=_NOTSET):
        ...
        if withscores:
            ...
            args = []
        fut = self.execute(b"ZRANGE", key, start, stop, *args, encoding=encoding)
        if withscores:
    ...
    def zrangebylex(
        ...
        return self.execute(b"ZRANGEBYLEX", key, min, max, *args, encoding=encoding)
    ...
    def zrangebyscore(
        ...
        *,
        ...
        if offset is not None and count is not None:
            args.extend([b"LIMIT", offset, count])
        fut = self.execute(b"ZRANGEBYSCORE", key, min, max, *args, encoding=encoding)
        if withscores:
            return wait_convert(fut, pairs_int_or_float)
        return fut
    ...
    def zremrangebyscore(
        self, key, min=float("-inf"), max=float("inf"), *, exclude=None
    ):
    ...
    def zrevrangebylex(
        ...
        return self.execute(b"ZREVRANGEBYLEX", key, max, min, *args, encoding=encoding)
    ...
    def zunionstore(self, destkey, key, *keys, with_weights=False, aggregate=None):
